...
def subgraph_nodes(all_features, senders, receivers, unions, uni_senders, senders_idxs, senders_counts, distances, dataset, area_room_name, batch_size=16, depth=3):
    ...
    for j in range(n_batches):
        ...
        for k in range(sampled_sp_idxs.shape[0]):
            fsenders, freceivers, _ = search_bfs_depth(vi=sampled_sp_idxs[k], edges=all_edges,
                distances=distances, direct_neigh_idxs=senders_idxs, n_edges=senders_counts, depth=depth)
...
def subgraph_nodes(all_features, senders, receivers, unions, uni_senders, senders_idxs, senders_counts, distances, dataset, area_room_name, batch_size=16, depth=3):
    n_unions = unions.shape[0]
    false_node_idxs = np.where(unions == False)[0]
    n_false_nodes = false_node_idxs.shape[0]
    n_true_nodes = n_unions - n_false_nodes
    true_node_idxs = np.delete(np.arange(n_unions), false_node_idxs)
    if n_false_nodes <= n_true_nodes:
        n_samples = n_false_nodes * 2
        true_node_idxs = true_node_idxs[:n_false_nodes]
    else:
        n_samples = n_true_nodes * 2
        false_node_idxs = false_node_idxs[:n_true_nodes]
    if false_node_idxs.shape[0] != true_node_idxs.shape[0]:
        raise Exception("True and false idxs have different shapes ({0}, {1})".format(false_node_idxs.shape[0], true_node_idxs[0]))
    if n_samples <= batch_size:
        return
    n_batches = math.floor(n_samples / batch_size)
    sample_idxs = np.arange(false_node_idxs.shape[0], dtype=np.uint32)
    np.random.shuffle(sample_idxs)
    b_half = math.floor(batch_size / 2)
    ...
    for j in range(n_batches):
        ...
        stop = start + b_half
        s_idxs = sample_idxs[start:stop]
        s_false_node_idxs = false_node_idxs[s_idxs]
        s_true_node_idxs = true_node_idxs[s_idxs]
        node_idxs = np.vstack((s_false_node_idxs[:, None], s_true_node_idxs[:, None]))
        node_idxs = node_idxs.reshape(node_idxs.shape[0], )

        sampled_unions = unions[node_idxs]

        senders_ = np.zeros((0, 1), dtype=np.uint32)
        receivers_ = np.zeros((0, 1), dtype=np.uint32)
        for k in range(node_idxs.shape[0]):
            fsenders, freceivers, _ = search_bfs_depth(vi=node_idxs[k], edges=all_edges,
                distances=distances, direct_neigh_idxs=senders_idxs, n_edges=senders_counts, depth=depth)
            senders_ = np.vstack((senders_, fsenders[:, None]))
            receivers_ = np.vstack((receivers_, freceivers[:, None]))
        senders_ = senders_.reshape(senders_.shape[0])
        ...
        all_inter_idxs = np.zeros((batch_size, ), dtype=np.int32)
        #print(senders_.shape[0])
        # we search the original nodes in the subgraph
        for k in range(batch_size):
            idx = node_idxs[k]
            idxs = np.where(all_nodes == idx)[0]
            all_inter_idxs[k] = idxs[0].astype(np.int32)
        tmp_node_features = np.zeros((all_nodes.shape[0], all_features.shape[-1]))
        mapping = 0
        mapped_senders = np.array(senders_, copy=True)
        mapped_receivers = np.array(receivers_, copy=True)
